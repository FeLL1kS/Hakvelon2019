...
class Logger {
    ...
    log(level, ...data) {
        if (!['info', 'error'].includes(level)) level = 'info';
    ...
    format(level, data) {
        ...
        let formated = (typeof this.options.format == 'function' ? this.options.format() : this.options.format.toString()).replace('%level%', level).replace('%source%', this.options.source).replace('%date%', this.options.date()).replace('%data%', message);
        return formated;
    }
}
class LoggerFactory {
